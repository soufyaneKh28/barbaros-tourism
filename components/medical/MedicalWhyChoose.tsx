...
const ICONS = [
    <svg key="jci" className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
    </svg>,
    <svg key="pricing" className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
    </svg>,
    <svg key="surgeons" className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <svg key="combination" className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
    </svg>,
    <svg key="aftercare" className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function MedicalWhyChoose({ locale }: MedicalWhyChooseProps) {
    ...
    const t = generic.medical_why_choose;

    const reasons = t.items.map((item: { title: string; description: string }, index: number) => ({
        ...
        <section className="py-20 bg-white relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {reasons.map((reason: { icon: React.ReactNode; title: string; description: string }, index: number) => (
                        <motion.div
                            key={index}
