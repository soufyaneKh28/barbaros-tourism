...
export default function Services({ services: dynamicServices }: ServicesProps) {
    const { t } = useLanguage();
    ...
    const displayServices = dynamicServices && dynamicServices.length > 0
        ? dynamicServices.map((service, index) => ({
            id: index,
            title: service.service_name,
            description: service.service_details,
            image: service.cover_image || '/images/generated/services_section_image.png',
            icon: (active: boolean) => (
                <svg className={`w-6 h-6 ${active ? 'text-white' : 'text-white'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
                </svg>
