...
export default function BookingCard({ price }: BookingCardProps) {
    ...
    // We can translate the WhatsApp message too, or keep it English/bilingual
    // Keeping it English-ish for now as requested by user often, but "per person" inside matches locale
    const perPersonText = tripDetails?.booking?.perPerson || 'per person';
    const whatsappMessage = `Hi, I'm interested in booking this tour. ${price ? `Price: $${price} ${perPersonText}.` : ''} Can you provide more details?`;
        ...
        <div className="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100 sticky top-24">
            {hasValidPrice && (
                <div className="flex items-end gap-2 mb-6">
                    ...
                    <span className="text-gray-500 font-satoshi mb-1">{perPersonText}</span>
                </div>
            )}
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-4 mb-6 border border-blue-100">
                <p className="text-sm text-gray-700 font-satoshi leading-relaxed">
                    {tripDetails?.booking?.contactText || 'Contact us on WhatsApp to book this tour and get personalized assistance for your travel plans.'}
                </p>
            </div>
            ...
            >
                ...
                {tripDetails?.booking?.bookWhatsApp || 'Book via WhatsApp'}
            </a>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
                <div className="flex items-center gap-2 text-sm text-gray-600 font-satoshi">
                    ...
                    <span>{tripDetails?.booking?.instantResponse || 'Instant response guaranteed'}</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-gray-600 font-satoshi mt-2">
                    <svg className="w-5 h-5 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span>{tripDetails?.booking?.flexibleOptions || 'Flexible booking options'}</span>
                </div>
            </div>
        </div>
    );
